...
static  func get_current_score_info(countries: Array[String], score: Array[int]) -> String:
	if score[0] == score[1]:
		return "EMPATE! %d - %d" % [score[0], score[1]]
	elif score [0] > score[1]:
		return "GANA %s %d - %d" % [countries[0], score[0], score[1]]
	else:
		return "GANA %s %d - %d" % [countries[1], score[1], score[0]]

static func get_final_score_info(countries: Array[String], score: Array[int]) -> String:
	if score [0] > score[1]:
		return "%s GANO %d - %d" % [countries[0], score[0], score[1]]
	else:
		return "%s GANO %d - %d" % [countries[1], score[1], score[0]]
